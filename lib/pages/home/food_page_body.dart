...
class _FoodPageBodyState extends State<FoodPageBody> {
                       ...
                       Container(
                         ...
                         decoration: BoxDecoration(
                             ...
                             image: DecorationImage(
                                 fit: BoxFit.cover,
                                 image: NetworkImage((AppConstants.BASE_URL+AppConstants.UPLOADS+recommendedProduct.recommendedProductList[index].img! ))),
                       ),
                       //text
                       ),Expanded(
                         child: Container(
  ...
  Widget _buildPageItem(int index, ProductModel popularProduct) {
    ...
    if (index == _currPageValue.floor()) {
      var currScale = 1 - (_currPageValue - index) * (1 - _scallFactor);
      var currTrans = _height * (1 - currScale) / 2;
      matrix = Matrix4.diagonal3Values(1, currScale, 1)
        ..setTranslationRaw(0, currTrans, 0);
    } else if (index == _currPageValue.floor() + 1) {
      var currScale =
          _scallFactor + (_currPageValue - index + 1) * (1 - _scallFactor);
      var currTrans = _height * (1 - currScale) / 2;
      matrix = Matrix4.diagonal3Values(1, currScale, 1);
      matrix = Matrix4.diagonal3Values(1, currScale, 1)
        ..setTranslationRaw(0, currTrans, 0);
      ...
      var currScale = 0.8;
      matrix = Matrix4.diagonal3Values(1, currScale, 1)
        ..setTranslationRaw(0, _height * (1 - _scallFactor) / 2, 1);
    }
    return Transform(
      transform: matrix,
      child: Stack(
